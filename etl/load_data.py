...
# creating a separate csv file for contact_person and contact columns along with the job_id column
df3 = pd.read_csv('data/completely_cleaned_jobs_data.csv')

cols = ['job_id', 'contact_person', 'contact']
df4 = df3[cols]

df4.to_csv('data/contact_info.csv', index=False)
print("Contacts File saved successfully!")
...
# have to work on remaining 3 tables (need to normalize the skills!)
import ast

# creating a separate csv file for skills and related tables i.e; job_details and job_Skill_map
# Convert string representation of list into actual list
skills = df3["skills"].apply(lambda x: [s.strip() for s in x.split(",")])

# Join list into comma-separated skill string
# df3['skills_cleaned'] = skills.apply(lambda lst: ", ".join(lst))

required_cols = [
   skills,
   df3['job_id'],
   df3['job_description'],
   df3['responsibilities'],
   df3['benefits'],
   df3['qualifications']
]

skills_job_details = pd.DataFrame({
    "skills": skills,
    "job_id": df3['job_id'],
    "job_description": df3['job_description'],
    "responsibilities": df3['responsibilities'],
    "benefits": df3['benefits'],
    "qualifications": df3['qualifications']
})

print("Skills and Job Details dataframe created successfully!")

skills_job_details.to_csv('data/skills_job_details.csv', index=False)
print("Skills and Job Details File saved successfully!")